...
type
  ...
  {功能:人工叫班}
  function MunualCall2(strCheCi:string = '';strRoomNum:string = ''):Boolean;
...
begin
  result := False;
  frm := TFrmMunualCall2.Create(nil);
  try
...
procedure TFrmMunualCall2.AddMemLog(strMsg: string);
begin

  mmoState.Lines.Add(TPubfun.DateTime2Str(Now) + ' : ' +strMsg);
end;

procedure TFrmMunualCall2.btnCloseClick(Sender: TObject);
begin
...
procedure TFrmMunualCall2.btnConnectClick(Sender: TObject);
var
  strMsg:string;
begin
...
procedure TFrmMunualCall2.btnDisconnectClick(Sender: TObject);
var
  strmsg:string;
begin
  if m_RoomCallApp.CallDevOp.bCalling then
...
procedure TFrmMunualCall2.btnSysVoiceClick(Sender: TObject);
var
  strMsg:string;
begin
{$IFNDEF UART_DEBUG}
...
procedure TFrmMunualCall2.edtRoomNumChange(Sender: TObject);
begin
  ReFreshData;
end;

procedure TFrmMunualCall2.FillGrid;
var
  i:Integer;
  item : TListItem;
begin
...
procedure TFrmMunualCall2.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  CanClose := False;
...
procedure TFrmMunualCall2.FormCreate(Sender: TObject);
begin
...
procedure TFrmMunualCall2.FormDestroy(Sender: TObject);
begin
...
procedure TFrmMunualCall2.FormShow(Sender: TObject);
begin
...
function TFrmMunualCall2.InitCallRecord:Boolean;
var
  dev:RCallDev;
...
procedure TFrmMunualCall2.InitData(strCheCi,strRoomNum:string);
begin
...
procedure TFrmMunualCall2.ListView1Click(Sender: TObject);
var
  CallDev : PCallDev ;
begin
  if ListView1.Selected = nil then
  ...
  CallDev :=  PCallDev(ListView1.Selected.Data);
  Self.Caption := '手工叫班: ' +  CallDev.strRoomNum ;
end;

procedure TFrmMunualCall2.OnDisConDevEvent(data: TCallDevCallBackData);
var
...
procedure TFrmMunualCall2.OnFinishConDev(data:TCallDevCallBackData);
var
  strMsg:string;
begin
  btnConnect.Enabled := True;
...
procedure TFrmMunualCall2.OnFinishFirstCallVoice(data:TCallDevCallBackData);
begin
  btnSysVoice.Enabled := True;
...
procedure TFrmMunualCall2.OnStartConDev(data:TCallDevCallBackData);
var
  strMsg:string;
begin
...
procedure TFrmMunualCall2.OnStartFirstCallVoice(data:TCallDevCallBackData);
begin
  AddMemLog('开始播放首叫语音');
end;

procedure TFrmMunualCall2.OnTryConDev(data: TCallDevCallBackData);
var
  strMsg:string;
begin
  case data.callRoomRecord.eCallResult of
...
procedure TFrmMunualCall2.ReFreshData;
var
...
procedure TFrmMunualCall2.Timer1Timer(Sender: TObject);
begin
  TTimer(Sender).Enabled := false ;
  btnConnect.Click ;
end;

procedure TFrmMunualCall2.tmrAutoCloseTimer(Sender: TObject);
begin
