...
begin
  ...
  begin
    ...
    if TextInput('乘务员身份验证','指纹识别失败,请输入乘务员工号:',strNumber,7) = False then
      Exit;
...
begin
  ...
  begin
    if TextInput('乘务员身份验证','指纹仪故障,请输入乘务员工号:',strNumber,7) = False then
    begin
      ModalResult := mrCancel;
      Exit;
    end;
  end else begin
    if TextInput('乘务员身份验证','没有找到匹配的指纹,请输入乘务员工号:',strNumber,7) = False then
    begin
      Exit;
    end;
  end;
